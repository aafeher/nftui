...
| **UDP / UDPLITE** | sport, dport, length (UDP) / csumcov (cobertura de checksum do UDP-Lite — a mesma célula no fio, renomeada a partir do contexto `meta l4proto`; `udplite length` não existe), checksum |
