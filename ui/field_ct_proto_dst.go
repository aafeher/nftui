...
// CtProtoDstField edits the ct proto-dst (connection tracking destination port) condition.
// Supports == / != operators and optional direction (original/reply).
type CtProtoDstField struct {
	opInput        Select
...
func NewCtProtoDstField(rd *nft.Rule) *CtProtoDstField {
	...
	for _, condition := range rd.Conditions {
		if condition.CT != nil && condition.CT.Key == nftexpr.CtKeyProtoDst {
...
func (f *CtProtoDstField) FocusSlots() int { return 3 }

func (f *CtProtoDstField) Focus(subIndex int) {
	switch subIndex {
	case 0:
		f.opInput.Focus()
	case 1:
...
func (f *CtProtoDstField) Save(rule *nftables.Rule) {
	if !f.Changed() {
		return
	}
	...
	for i, re := range rule.Exprs {
		if ct, ok := re.(*expr.Ct); ok && ct.Key == expr.CtKeyPROTODST {
			found = true
			ct.Direction = dir
	...
	if !found && newPort > 0 {
		...
		newExprs = append(newExprs,
			&expr.Ct{Key: expr.CtKeyPROTODST, Register: 1, Direction: dir, OptDirection: optDir},
...
func (f *CtProtoDstField) View() string {
	vOp := f.opInput.View()
	if f.opChanged() {
	...
	return grayStyle.Render("CT proto-dst") + "\n" + inputs + "\n"
}
